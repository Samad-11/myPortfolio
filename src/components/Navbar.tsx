...
import { SheetContent, SheetDescription, SheetHeader, SheetTrigger, SheetClose } from './ui/sheet'
                                ...
                                <SheetDescription >
                                    ...
                                    <ul className='flex flex-col justify-center items-center gap-5 border border-r-amber-50 h-screen items-end border-none text-slate-100 '>
                                        {links.map((link) => (
                                            <li key={link.path}>
                                                <SheetClose asChild>
                                                    <Link href={link.path} className={`${pathname == link.path && 'text-stone-400'} hover:text-stone-400 font-semibold font-mono`}>{link.name}</Link>
                                                </SheetClose>
                                            </li>
                                        ))}
                                    </ul>
